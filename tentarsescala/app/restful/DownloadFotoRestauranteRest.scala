...
object DownloadFotoRestauranteRest extends Controller {
...
//                                         "v2" ->  JsString(x.infoFoto.valor2)
	...
	def mostrartFotoRestaurante(fName: String) = Action { implicit request =>  
		//println(fName);
		val in:Either[Exception,(InputStream,String,Long)] =CloudStorage.downloadRestaurante(fName);
		in.fold((ex)=>{  
		    Status(503)(
		        JsObject(
		            Seq("mensaje"->JsString("error cargar archivo"))
  ...
  def mostrartFotoRestauranteThumbnail(fName: String) = Action { implicit request =>  
		//println(fName);
		val in:Either[Exception,(InputStream,String,Long)] =CloudStorage.downloadRestauranteThumbnail(fName);
		in.fold((ex)=>{  
		    Status(503)(
		        JsObject(
		            Seq("mensaje"->JsString("error cargar archivo"))
