...
object DependenciaClase extends App{
   ...
   val callClases= paresElementoClases.collect().foldLeft(List[String]())((z,x)=> {
      ...
      val rpp = BusquedaArchivos.busquedaCall(pathFuentes+"/"+x._4)//(sc)  ;
      if(rpp!=null)
      {
    ...
    val callModulo= paresElementoModulos.collect().foldLeft(List[String]())((z,x)=> {
      ...
      val rpp = BusquedaArchivos.busquedaCall(pathFuentes+"/"+x._4)  ;
      if(rpp!=null)
      {
        val lista =rpp.toList.map { (y) => {
           x._3+";"+y
   ...
   val callDesigner= paresElementoDesigners.collect().foldLeft(List[String]())((z,x)=> {
    ...
    linesForm.distinct().saveAsTextFile("/home/x4/libreria/sbt/AnalisisCajaSpark/salida/visual/dependencia/formAnalisis")
    
    
    val linesDesigner = sc.parallelize(callDesigner)
    linesDesigner.distinct().saveAsTextFile("/home/x4/libreria/sbt/AnalisisCajaSpark/salida/visual/dependencia/designerAnalisis")
    
    
    val linesClass = sc.parallelize(callClases)
    linesClass.distinct().saveAsTextFile("/home/x4/libreria/sbt/AnalisisCajaSpark/salida/visual/dependencia/clasesAnalisis")
    
    
    val linesModulo = sc.parallelize(callModulo)
    linesModulo.distinct().saveAsTextFile("/home/x4/libreria/sbt/AnalisisCajaSpark/salida/visual/dependencia/moduloAnalisis")
