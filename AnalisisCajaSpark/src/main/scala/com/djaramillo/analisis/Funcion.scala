...
object Funcion extends App{
   ...
   val callClases= paresElementoClases.collect().foldLeft(List[String]())((z,x)=> {
      ...
      val rpp = BusquedaArchivos.busquedaFunction(pathFuentes+"/"+x._4)//(sc)  ;
      if(rpp!=null)
      {
    ...
    val callModulo= paresElementoModulos.collect().foldLeft(List[String]())((z,x)=> {
      ...
      val rpp = BusquedaArchivos.busquedaFunction(pathFuentes+"/"+x._4)  ;
      if(rpp!=null)
      {
        val lista =rpp.toList.map { (y) => {
           x._3+";"+y
   ...
   val callForm= paresElementoForms.collect().foldLeft(List[String]())((z,x)=> {
    ...
    linesForm.saveAsTextFile("/home/x4/libreria/sbt/AnalisisCajaSpark/salida/visual/function/formFunction")
    
    
    val linesDesigner = sc.parallelize(callDesigner)
    linesDesigner.saveAsTextFile("/home/x4/libreria/sbt/AnalisisCajaSpark/salida/visual/function/designerFunction")
    
 
    val linesClass = sc.parallelize(callClases)
    linesClass.saveAsTextFile("/home/x4/libreria/sbt/AnalisisCajaSpark/salida/visual/function/clasesFunction")
    
    
    val linesModulo = sc.parallelize(callModulo)
    linesModulo.saveAsTextFile("/home/x4/libreria/sbt/AnalisisCajaSpark/salida/visual/function/moduloFunction")
