...
object BaseDeDatos extends App{
   ...
   val callClases= paresElementoClases.collect().foldLeft(List[String]())((z,x)=> {
      ...
      val rpp = BusquedaArchivos.buscarBaseDatos(pathFuentes+"/"+x._4)//(sc)  ;
    ...
    val callModulo= paresElementoModulos.collect().foldLeft(List[String]())((z,x)=> {
      ...
      val rpp = BusquedaArchivos.buscarBaseDatos(pathFuentes+"/"+x._4)  ;
   ...
   val callForm= paresElementoForms.collect().foldLeft(List[String]())((z,x)=> {
    ...
    linesForm.saveAsTextFile("/home/x4/libreria/sbt/AnalisisCajaSpark/salida/visual/db/formBD")
    
    
    val linesDesigner = sc.parallelize(callDesigner)
    linesDesigner.saveAsTextFile("/home/x4/libreria/sbt/AnalisisCajaSpark/salida/visual/db/designerBD")
    
 
    val linesClass = sc.parallelize(callClases)
    linesClass.saveAsTextFile("/home/x4/libreria/sbt/AnalisisCajaSpark/salida/visual/db/clasesBD")
    
    
    val linesModulo = sc.parallelize(callModulo)
    linesModulo.saveAsTextFile("/home/x4/libreria/sbt/AnalisisCajaSpark/salida/visual/db/moduloBD")
